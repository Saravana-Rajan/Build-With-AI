# Sevai Setu (சேவை சேது) — Design Spec
...
**Working name:** Sevai Setu (சேவை சேது) — "Bridge of Service."
...
BigQuery  ── demand records  +  public datasets (Census/UDISE/NFHS/PMAY/JJM/MGNREGA, joined on LGD codes)
        ...
        │            │
        ▼            ▼
Cloud Run API (FastAPI)  →  Next.js dashboard (Maps Platform) + Gemini-drafted letters/memos
                              Earth Engine imagery for hero villages
```

All serverless, all on Google Cloud credits.
